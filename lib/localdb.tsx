...
const LocalDB = {
  load: function (): DBSchemaV1 {
    ...
    if (value !== null) {
      try {
        ...
        console.warn(
          "Invalid JSON in localStorage :( Reverting to v1 defaults",
        );
        console.info(value);
      }
    }
  ...
  addFavorite: function (blueprint: BlueprintInfo, address: string): undefined {
    ...
    const currentIndex = state.favorites.findIndex(
      (favorite) => favorite.blueprint.originalHref === blueprint.originalHref,
    );
    if (currentIndex > -1) {
      return;
    }
  ...
  isFavorite: function (blueprint: BlueprintInfo): boolean {
    return (
      -1 <
      LocalDB.getFavorites().findIndex(
        (favorite) =>
          favorite.blueprint.originalHref === blueprint.originalHref,
      )
    );
  },
  ...
  addRecentlyViewed: function (
    ...
    state.recentlyViewed = state.recentlyViewed.filter(
      (recent) => recent.blueprint.originalHref !== blueprint.originalHref,
    );
