...
const randIndex = (array: Array<any>) =>
  Math.trunc(Math.random() * array.length);
const randItem = (array: Array<any>) => array[randIndex(array)];

export default function Frontpage() {
  const { addresses, setCurrentBlueprint } =
    useOutletContext<AppOutletContextType>();
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      let blueprints = await Promise.all(
        randomAddresses.map(async (addrinfo) => {
          let response = await fetch(
            `${DATA_URL_PREFIX}/addresses/${addrinfo.address}.json`,
          );
    ...
    <div>
      {favorites.length > 0 && (
        <>
          ...
          <div className={overviewStyles.overviewContainer}>
            {favorites.map(
              ({ address, blueprint }: AddressAndBlueprintInfo) => (
                <BlueprintCardLink
                  address={address}
                  blueprint={blueprint}
                  key={blueprint.hash}
      ...
      {recentlyViewed.length > 0 && (
        <>
          ...
          <div className={overviewStyles.overviewContainer}>
            {recentlyViewed
              .slice(0, MAX_RECENTLY_VIEWED_RESULTS)
              .map(({ address, blueprint }: AddressAndBlueprintInfo) => (
                <BlueprintCardLink
                  address={address}
                  blueprint={blueprint}
                  key={blueprint.hash}
